...
{
	int len = n, i = 2;
	if (n%2) len -= 2; else len--; // podzielenie na 2 r�wne czesci wymaga parzystej liczby
	if (t[0] <= t[1]){
		min = t[0];
		max = t[1];
	}else{
	...
	while (i < len){
		if (t[i] <= t[i+1]){ // metoda dziel i zwyciezaj polega na podziale tabeli na 2 mniejsze a nastepnie znalezienia ekstrem�w z tablicy o kt�rej wiadomo ze w niej sie pojawi
	...
	if (n%2) // w przypadku nieparzystego n ostatni element zostje por�wnany osobno
	{
